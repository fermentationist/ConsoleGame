...
const Commands = function (game: GameType) : CommandAlias[] {
	...
	const _inventory = (command: string) => {
		...
		let items: string[] = [], itemsPlusArticles: string[] = [];
  ...
  const _quit = () => {
	...
	const aliasString = (word: string, thesaurus?: Record<string, string[]>, optionalString = "") => {
	...
	const itemAliases: CommandAlias[] = itemNames.map(itemName => [_items, aliasString(itemName, thesaurus)]);
